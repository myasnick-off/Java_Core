...
public class Cat implements Movable {
    ...
    public boolean jump(Wall wall) {
        if (wall.getHeight() > maxHeight) {
            System.out.printf("This wall too high for %s! Cat out of the race!\n", name);
            return false;
        }
        System.out.printf("Cat %s jumped over %d meter(s)\n", name, wall.getHeight());
        return true;
    }
    ...
    public boolean run(Track track) {
        if (track.getLength() > maxDistance) {
            System.out.printf("This track too long for %s! Cat out of the race!\n", name);
            return false;
        }
        System.out.printf("Cat %s had run %d meters\n", name, track.getLength());
        return true;
    }
}
